...
export default function connect<SourceT extends {}>(
    component:
        | React.ComponentClass<SourceT>
        | React.StatelessComponent<SourceT>,
    source: OpStream<SourceT>
): React.ComponentClass<SourceT>
export default function connect<
    OriginalPropsT extends {},
    SourceT extends {},
    ResultPropsT extends {}
>(
    component:
        | React.ComponentClass<ResultPropsT>
        | React.StatelessComponent<ResultPropsT>,
    source: OpStream<SourceT>,
...
export default function connect<T, U, V extends {}>(
    ...
    const instances: ImmviewConnector[] = []
    ...
    class ImmviewConnector extends React.Component<
        T,
        { sourceReceived: boolean; sourceValue?: U }
    > {
        state = { sourceReceived: false }
        componentWillMount() {
            instances.push(this as ImmviewConnector)
        }

        componentWillUnmount() {
            const pos = instances.indexOf(this as ImmviewConnector)
            if (pos >= 0) {
